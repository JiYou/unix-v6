...
char	name[16];		//用户输入的登录名
int	crmod;
int	upper;
int	lower;

main(argc, argv)//argv 不包括第一个参数exe file
char **argv;
{
...
*/
	tname = '0';
	if (argc > 1)
		tname = *argv[1];	//p->comn[0]
	for (;;) {
		for(tabp = itab; tabp < &itab[NITAB]; tabp++)	//这段循环可否挪到循环外?
			if(tabp->tname == tname)
				break;
		if(tabp >= &itab[NITAB])
			tabp = itab;
		...
		if(getname()) {
			...
			if(lower)
				tmode.sgflag =& ~LCASE;
			stty(0, &tmode);	//设置从表中找到的tty,然后运行登录程序，处理用户登录
			execl("/bin/login", "login", name, 0);	//name是用户输入的登录名
			exit(1);
		}//否则用户输入了'\0'?
		tname = tabp->nname;
	}
